#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <string.h>
#include "../include/command.h"

/**
 * @brief Read the user's input string
 * 
 * @return char* 
 * Return string
 */
char *read_line()
{
    char *buffer = (char *)malloc(BUF_SIZE * sizeof(char));
    if (buffer == NULL) {
        perror("Unable to allocate buffer");
        exit(1);
    }

	if (fgets(buffer, BUF_SIZE, stdin) != NULL) {
		if (buffer[0] == '\n' || buffer[0] == ' ' || buffer[0] == '\t') {
			free(buffer);
			buffer = NULL;
		} 
		else {
			buffer[strcspn(buffer, "\n")] = 0;
			strncpy(history[history_count % MAX_RECORD_NUM], buffer, BUF_SIZE);
			++history_count;
		}
	}

	return buffer;
}

/**
 * @brief Parse the user's command
 * 
 * @param line User input command
 * @return struct cmd* 
 * Return the parsed cmd structure
 */
struct cmd *split_line(char *line)
{
    int args_length = 10;
    struct cmd *new_cmd = malloc(sizeof(struct cmd));
    struct cmd_node *cur = malloc(sizeof(struct cmd_node));

    new_cmd->head = cur;
    cur->args = malloc(args_length * sizeof(char *));
    for (int i = 0; i < args_length; i++)
        cur->args[i] = NULL;

    cur->length = 0;
    cur->in_file = NULL;
    cur->out_file = NULL;
    cur->in = 0;
    cur->out = 1;
    cur->next = NULL;

    new_cmd->pipe_num = 1;

    char *token = strtok(line, " ");

    while (token != NULL)
    {
        // -------------------------
        // case 1: pipe |
        // -------------------------
        if (strcmp(token, "|") == 0)
        {
            // finish current cmd_node
            cur->args[cur->length] = NULL;

            // create new node
            struct cmd_node *next = malloc(sizeof(struct cmd_node));
            next->args = malloc(args_length * sizeof(char *));

            for (int i = 0; i < args_length; i++)
                next->args[i] = NULL;

            next->length = 0;
            next->in_file = NULL;
            next->out_file = NULL;
            next->in = 0;
            next->out = 1;
            next->next = NULL;

            cur->next = next;
            cur = next;

            new_cmd->pipe_num++;
        }
        // -------------------------
        // case 2: input redirection <
        // -------------------------
        else if (strcmp(token, "<") == 0)
        {
            token = strtok(NULL, " ");
            if (token)
                cur->in_file = token;
        }
        // -------------------------
        // case 3: output redirection >
        // -------------------------
        else if (strcmp(token, ">") == 0)
        {
            token = strtok(NULL, " ");
            if (token)
                cur->out_file = token;
        }
        // -------------------------
        // case 4: normal arg
        // -------------------------
        else
        {
            cur->args[cur->length] = token;
            cur->length++;
        }

        token = strtok(NULL, " ");
    }

    return new_cmd;
}

/**
 * @brief Information used to test the cmd structure
 * 
 * @param cmd Command structure
 */
void test_cmd_struct(struct cmd *cmd)
{
	struct cmd_node *temp = cmd->head;
	int pipe_count = 0;
	printf("============ COMMAND INFO ============\n");
	while (temp != NULL) {
		printf("cmd_node %d: ", pipe_count);
		for (int i = 0; i < temp->length; ++i) {
			printf("%s ", temp->args[i]);
		}
		printf("\n");
		temp = temp->next;
		++pipe_count;
	}
	printf("============ COMMAND INFO END ============\n");
}

/**
 * @brief Information used to test the cmd_node structure
 * 
 * @param temp cmd_node structure
 */
void test_pipe_struct(struct cmd_node *temp){
	printf("============ CMD_NODE INFO ============\n");
	
	for (int i = 0; i < temp->length; ++i) {
		printf("temp->args[%d] :%s \n",i, temp->args[i]);
	}
	printf(" in-file: %s\n", temp->in_file ? temp->in_file : "none");
	printf("out-file: %s\n", temp->out_file ? temp->out_file : "none");
	printf(" in: %d\n", temp->in );
	printf("out: %d\n", temp->out);
	printf("============ CMD_NODE END ============\n");
}
